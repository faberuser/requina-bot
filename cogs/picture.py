        ...
        if option is not None:
            if str(option) == 'file':
                ...
                if option.startswith('<@'):
                    user = (((option.replace('<','').replace('>','')).replace('@','')).replace('!',''))
                ...
                with open('./data/kiss.txt', 'r') as f:
                    ...
                    if str(option).endswith(' last'):
                        embed = discord.Embed(title=f'`{name[:-5]}` you have been kissed by `{ctx.author.name}`')
                        embed.set_image(url=re[-1])
                    else:
                        embed = discord.Embed(title=f'`{name}` you have been kissed by `{ctx.author.name}`')
                        embed.set_image(url=str(random.choice(re)))
                    await ctx.send(embed=embed)
        else:
