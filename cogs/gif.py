...
class Gif(commands.Cog):
    ...
    async def gif(self, ctx, *, gname: str = None):
        ...
        if gname is None:
            ...
            embed = discord.Embed(
                title="List of Gifs", description=f"{str(gif2)[1:][:-1]}"
            )
    ...
    @commands.command(aliases=["gif_random", "random_gif", "gifrandom", "randomgif"])
    ...
    @commands.command(aliases=["gif_add", "add_gif", "addgif"])
    async def gifadd(self, ctx, gname: str = None, gurl: str = None):
        if (
            ctx.author.id == 315724989057990663
            or ctx.author.id == 213186434193031168
            or ctx.author.id == 470081764271063060
        ):
    ...
    @commands.command(aliases=["gif_remove", "remove_gif", "removegif"])
    async def gifremove(self, ctx, gname: str = None):
        if (
            ctx.author.id == 315724989057990663
            or ctx.author.id == 213186434193031168
            or ctx.author.id == 470081764271063060
    ...
    @gifadd.error
    async def gifadd_error(self, ctx, error):
    ...
    @gifremove.error
    async def gifremove_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            await ctx.send("You don't have enough Permission(s) to use this command.")
        if isinstance(error, commands.MissingRequiredArgument):
            await ctx.send("Missing Argument Error")
