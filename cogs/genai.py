...
class GenAI(commands.Cog):
    ...
    @commands.command(aliases=['summarise', 'summary', 'sum'])
    async def summarize(self, ctx, history_length: int = 100):
        ...
        try:
            ...
            messages.reverse()
            text = "\n".join([msg.author.display_name + ": " +
                             msg.content for msg in messages if msg.content and not msg.author.bot and not msg.content.lower().startswith("r.")])
            if not text:
                await bot_msg.edit(content="No messages to summarize.")
                return
            response = client.models.generate_content(
                model="gemini-flash-latest", contents="Tóm tắt cuộc trò chuyện này:" + "\n\n" + text
            )
            embed = discord.Embed(
                title=f"Summary for Last {str(history_length)} Messages", description=response.text, color=config.embed_color)
